...
def save_log(db: Session, request: LogRequest):
    ...
    forecast_time = datetime.fromisoformat(request.forecast_for) if request.forecast_for else datetime.now(timezone.utc).replace(tzinfo=None)
...
def get_statistics(db: Session, location_name: str, days: int) -> list[StatisticItem]:
    ...
    records = db.query(CloudMetricsLog).filter(
        ...
        CloudMetricsLog.forecast_for <= datetime.now(timezone.utc).replace(tzinfo=None),
...
def cleanup_old_records(db: Session):
    ...
    cutoff_history = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(days=30)
    ...
    cutoff_forecast = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(hours=12)
